...
function config(app, { showLoginPage, serviceProvider, isStateless }) {
  ...
  app.post(
    ...
    (req, res) => {
      console.info('[sgid:browser-debug]', req.body)
      res.status(204).end()
    },
  )
  ...
  app.get(`${PATH_PREFIX}/authorize`, (req, res) => {
    ...
    console.info('[sgid:authorize:start]', {
      clientId,
      redirectHost: getUrlHost(redirectURI),
      redirectLength: redirectURI?.length,
      stateLength: state?.length,
      noncePresent: !!nonce,
      scopes,
      showLoginPage: showLoginPage(req),
    })
    if (showLoginPage(req)) {
      ...
      console.info('[sgid:authorize:render-login]', {
        optionCount: values.length,
        firstOption: values[0]?.id,
      })
      ...
      const assertURL = buildAssertURL(redirectURI, authCode, state)
      console.info('[sgid:authorize:auto-redirect]', {
        clientId,
        profile: idGenerator.singPass(profile),
        redirectHost: getUrlHost(assertURL),
        redirectLength: assertURL.length,
      })
  ...
  app.post(
    ...
    (req, res) => {
      const returnTo = req.body?.returnTo || req.originalUrl

      res.redirect(
        303,
        `/unauthorised.html?returnTo=${encodeURIComponent(returnTo)}`,
      )
    },
  )
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      const aud = req.body.client_id || getBasicAuthClientId(req)

      console.info('[sgid:token:start]', {
        authCodePresent: !!authCode,
        audienceFromRequest: aud,
        usedBasicAuth: !!getBasicAuthClientId(req),
        redirectHost: getUrlHost(req.body.redirect_uri),
      })
      ...
      try {
        ...
        } = lookUpByAuthCode(authCode, {
          isStateless,
        })
        console.info('[sgid:token:auth-code-found]', {
          profile: idGenerator.singPass(profile),
          scopes,
          storedClientId,
        })
        const accessToken = authCode
        const audience = aud || storedClientId
        ...
        addMinimalProfileClaims(idTokenClaims, profile)

        delete idTokenClaims.email
        delete idTokenClaims.email_verified

        console.info('[sgid:token:id-token-claims]', {
          iss: idTokenClaims.iss,
          aud: idTokenClaims.aud,
          sub: idTokenClaims.sub,
          hasName: !!idTokenClaims.name,
          hasPhoneNumber: !!idTokenClaims.phone_number,
        ...
        console.info('[sgid:token:response]', {
          idTokenLength: idToken.length,
          accessTokenLength: accessToken.length,
          scopeLength: scopes?.length,
        })
        res.json({
  ...
  app.get(`${PATH_PREFIX}/userinfo`, async (req, res) => {
    ...
    console.info('[sgid:userinfo:start]', {
      authCodePresent: !!authCode,
    ...
    console.info('[sgid:userinfo:profile]', {
      profile: idGenerator.singPass(profile),
      scopes,
    })
    ...
    console.info('[sgid:userinfo:scopes]', {
      scopesArr,
    ...
    console.info('[sgid:userinfo:response]', {
      sub: `u=${uuid}`,
      encryptedFieldCount: Object.keys(data).length,
    })
  })
...
const getBasicAuthClientId = (req) => {
  const authHeader = req.headers.authorization || req.headers.Authorization
  if (!authHeader?.startsWith('Basic ')) {
    return undefined
  }

  const credentials = Buffer.from(authHeader.slice('Basic '.length), 'base64')
    .toString('utf8')
    .split(':')

  return decodeURIComponent(credentials[0])
}
...
const addMinimalProfileClaims = (idTokenClaims, profile) => {
  const claims = profile.claims || {}
  idTokenClaims.name = claims.name?.value
  idTokenClaims.phone_number = formatPhoneNumber(claims.mobileno)
}
